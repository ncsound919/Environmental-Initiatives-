...
Unified Data Layer (Postgres/TimescaleDB/Neo4j/Redis)
   ↓                               │
...
- **Deployment:** Favor `docker-compose up` for end-to-end bring-up (Keycloak, Kafka, API, web). Ensure `.env` includes broker URLs, Postgres/Neo4j/Redis endpoints, and Keycloak realm settings matching the EcoSphere defaults.
...
|--------------|-----------------------------------|------------------------------|----------------|-----------------------|
| Sensor/Edge Node (ESP32/STM32) | `ecos/{project}/{deviceId}/telemetry` with `TelemetrySchema` (`sensorId`, `measurementType`, `measurementValue`, `unit`, `timestamp`, `schemaVersion`) | `iot.sensor.telemetry` → project-specific domain topic (e.g., `agriculture.yield.greenhouse`, `water.generation.atmospheric`) | TimescaleDB (metrics) + Postgres (device registry) | Commands on `ecos/{project}/{deviceId}/command` (e.g., setpoint updates) |
...
| Edge Compute (Raspberry Pi/Jetson) | `ecos/{project}/{deviceId}/telemetry` (camera/ML summaries) | `ml.prediction.generated` / `compliance.audit.required` | Postgres (events) + object storage pointer if needed | `.../command` for model version/pipeline toggles |
| Power & Metering (CT clamps/smart meters) | `ecos/{project}/{deviceId}/telemetry` (kW/kWh, voltage) | `energy.production.solar`, `energy.consumption.thermal`, `commerce.transaction.completed` | TimescaleDB for curves; Postgres for billing snapshots | `.../command` for demand-response throttling |
...
- Telemetry from at least one device per project flows MQTT → Kafka → TimescaleDB/Postgres and is visible on the Operations Console.
