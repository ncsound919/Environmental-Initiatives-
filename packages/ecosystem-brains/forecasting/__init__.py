...
Heavy dependencies (prophet, torch) are imported lazily so this package can be
imported and the non-ML helpers used in minimal environments. Functions that
require a missing dependency raise a clear ImportError instead of crashing at
import time.
"""
...
except ImportError:  # pragma: no cover - exercised only without prophet
...
if nn is not None:
    ...
    class LSTMForecaster:  # type: ignore[no-redef]
        """Placeholder raised when torch is not installed."""

        def __init__(self, *args, **kwargs):
            raise ImportError(
                "torch is required for LSTMForecaster. Install it with: pip install torch"
            )
...
class ProphetForecaster:
    ...
    def __init__(self, seasonality_mode: str = 'multiplicative'):
        if Prophet is None:
            raise ImportError(
                "prophet is required for ProphetForecaster. Install it with: pip install prophet"
            )
...
def predict_bulb_failure(telemetry_data: Dict[str, float]) -> Dict[str, float]:
    """
    Heuristic reliability score for Centennial Bulb (#8).

    NOTE: This is NOT a Bayesian model. It is a transparent, hand-weighted
    stress score used as a placeholder until a real reliability model is
    trained and validated against field failure data. The returned
    `model_type` field makes this explicit for downstream consumers.

    Args:
        telemetry_data: Dict with 'voltage', 'thermal_cycles', 'uptime'

    Returns:
        Failure probability, expected remaining lifetime, and model type
    """
